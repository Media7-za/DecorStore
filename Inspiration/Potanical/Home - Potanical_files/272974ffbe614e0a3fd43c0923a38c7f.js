...
var isProvinceChange = !1;
var isBuildingPage = !0;
jQuery(document).ready(function () {
  ...
  if (jQuery(":hidden#billing_city").length > 0) {
    select2fields = { location_type: "Select your location type" };
  ...
  jQuery.each(select2fields, function (field, placeholder) {
    ...
    try {
      el.select2({ placeholder: placeholder, width: "100%" });
    } catch (err) {
      console.log(err);
    }
  });
  if (!jQuery(":hidden#billing_city").length > 0) {
    var ajaxUpdates = [
      ...
      {
        fromField: "billing_city",
        field: "billing_city_int",
        prefix: "suburbs",
        db_prefix: "billing",
      },
      {
        fromField: "shipping_state",
        field: "shipping_city",
        prefix: "towns",
        db_prefix: "shipping",
      },
      {
        fromField: "shipping_city",
        field: "shipping_suburb",
        prefix: "suburbs",
        db_prefix: "shipping",
      },
    ];
  ...
  jQuery('form[name="checkout"]').submit(function (event) {
    if (
      jQuery("#billing_country").val() != "ZA" ||
      (jQuery("#shipping_country").val() != "ZA" &&
        jQuery("#ship-to-different-address input:checked").length > 0)
    ) {
...
function updateInternational(type) {
  ...
  if (isChange) {
    ...
    if (fromSelect2) fromSelect2.close();
    removeInlineStyling(type);
    if (fromEl.val() == "ZA") {
      setInternational(type, !0);
...
function updateSelect(fromField, field, prefix, db_prefix) {
  ...
  var fromEl = jQuery("#" + fromField),
    ...
    isChange =
      (fromEl.val() !== "" && fromEl.val() != colliveryFieldsValues[fromField]) ||
      saProvinces.includes(fromEl.val()),
  ...
  if (fromField.indexOf("state") != -1 && isChange) {
    cacheValue(field, "");
    resetSelect(
      jQuery("#" + db_prefix + "_suburb"),
      '<option selected="selected" value="">First Select Town...</option>',
    );
  }
  if (fromSelect2) {
    fromSelect2.close();
  ...
  if (isChange) {
    ...
    return (ajax = jQuery.ajax({
      ...
      error: function () {
        resetSelect(
          el,
          '<option selected="selected" value="">Error retrieving data from server. Please refresh the page and try again</option>',
        );
      },
      beforeSend: function () {
