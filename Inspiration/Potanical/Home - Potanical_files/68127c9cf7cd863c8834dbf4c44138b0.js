(function ($) {
  "use strict";
  const PIXEL_SDK_WAIT_CONFIG = { initialDelayMs: 100, maxDelayMs: 5000, maxAttempts: 20 };
  const MailchimpPixelTracking = {
    ...
    waitForPixelSDK: function (options) {
      ...
      const initialDelayMs =
        config.initialDelayMs !== undefined
          ...
          : PIXEL_SDK_WAIT_CONFIG.initialDelayMs;
      const maxDelayMs =
        config.maxDelayMs !== undefined ? config.maxDelayMs : PIXEL_SDK_WAIT_CONFIG.maxDelayMs;
      const maxAttempts =
        config.maxAttempts !== undefined ? config.maxAttempts : PIXEL_SDK_WAIT_CONFIG.maxAttempts;
      function isSDKReady() {
        return (
    ...
    getRestBase: function () {
      return (
        (window.mcPixelConfig && window.mcPixelConfig.restBase) ||
        ...
        switch (eventType) {
          case "PRODUCT_ADDED_TO_CART":
            if (data.added_to_cart) {
              var atcItems = Array.isArray(data.added_to_cart)
                ? data.added_to_cart
                : [data.added_to_cart];
          ...
          case "PRODUCT_REMOVED_FROM_CART":
            if (data.removed_from_cart) {
              var rfcItems = Array.isArray(data.removed_from_cart)
          ...
          case "PRODUCT_VIEWED":
            if (data.product) {
              ...
              const atcArr = Array.isArray(data.added_to_cart)
                ? data.added_to_cart
                : data.added_to_cart
                  ? [data.added_to_cart]
                  : [];
              const rfcArr = Array.isArray(data.removed_from_cart)
                ? data.removed_from_cart
                : data.removed_from_cart
                  ? [data.removed_from_cart]
                  : [];
              const atcMatch = atcArr.some(function (p) {
                return String(p.productId || p.id) === viewedId;
              });
              const rfcMatch = rfcArr.some(function (p) {
                return String(p.productId || p.id) === viewedId;
              });
              if (
          ...
          case "CHECKOUT_STARTED":
            if (data.checkout) {
              ...
              window.mcPixel._handled.checkout = !0;
            }
            break;
          case "PURCHASED":
            if (data.order) {
          ...
          case "PRODUCT_CATEGORY_VIEWED":
            if (data.category) {
              ...
              window.mcPixel._handled.category = !0;
            }
            break;
          case "SEARCH_SUBMITTED":
            if (data.search) {
              this.sendSearchSubmitted(data.search);
              window.mcPixel._handled.search = !0;
            }
            break;
        }
      });
    ...
    sendProductViewed: function (product) {
      ...
      window.$mcSite.pixel.api.track("PRODUCT_VIEWED", { product: product }).catch((error) => {
    ...
    sendCartViewed: function (cart) {
      if (!this.isPixelSDKReady()) return;
      window.$mcSite.pixel.api.track("CART_VIEWED", { cart: cart }).catch((error) => {
        console.error("Mailchimp Pixel: Error tracking CART_VIEWED", error);
      });
    },
    sendCheckoutStarted: function (checkout) {
      if (!this.isPixelSDKReady()) return;
      window.$mcSite.pixel.api.track("CHECKOUT_STARTED", { checkout: checkout }).catch((error) => {
    ...
    sendPurchased: function (order) {
      if (!this.isPixelSDKReady()) return;
      window.$mcSite.pixel.api.track("PURCHASED", { order: order }).catch((error) => {
              ...
              var url =
                typeof input === "string"
                  ...
                  : input instanceof Request
                    ...
                    : String(input);
              if (
                /wc\/store\/v1\/cart\/add-item/.test(url) ||
                /wc\/store\/v1\/cart\/update-item/.test(url)
              ) {
                self.fetchAndTrackAddToCart();
