...
function mailchimpHandleBillingEmail(selector) {
  try {
    ...
    d.onload = function () {
      ...
      var msg = successful
        ? "mailchimp.handle_billing_email.request.success"
        : "mailchimp.handle_billing_email.request.error";
      if (successful) {
    ...
    d.send(
      "email=" +
        b +
        "&mc_language=" +
        mailchimp_public_data.language +
        "&subscribed=" +
        (subscribed && subscribed.checked ? "1" : "0"),
    );
    return true;
  } catch (a) {
  ...
  var g = {
    ...
    h = (function (a, b) {
      ...
      return (
        (c.get = function (b, d) {
          return (
            a.cookie !== c._cacheString && c._populateCache(),
            void 0 == c._cache[b] ? d : c._cache[b]
          );
        }),
        (c.defaults = { path: "/", secure: true, samesite: "strict" }),
...
mailchimpReady(function () {
  ...
  try {
    ...
    var mailchimp_username_email_block = document.querySelector(
    ...
    if (mailchimp_username_email_block) {
      mailchimp_username_email_block.onblur = function () {
        mailchimpHandleBillingEmail('#contact-fields input[type="email"]');
      };
      mailchimp_username_email_block.onfocus = function () {
        mailchimpHandleBillingEmail('#contact-fields input[type="email"]');
      };
      var typingTimer;
